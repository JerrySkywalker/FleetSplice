...
function Desktop() {
  ...
  return <main>
    ...
    <section><header><div><h2>{section}</h2><p className="muted">{message}</p></div><button onClick={() => void refresh()}>Refresh</button></header>
      ...
      {section === 'Gateway' && <article><b>Connection state</b><p>Hub: {status.hub ?? 'Unavailable'} · Edge: {status.edge ?? 'Unavailable'} · Admission: {status.edgeAdmission ?? 'Unavailable'}</p><label>Workspace for this Agent<input value={workspace} onChange={event => setWorkspace(event.target.value)} placeholder="C:\\src\\your-workspace" /></label><button onClick={async () => { try { const next = await invoke<AgentStatus>('onboard_agent', { workspace }); setStatus(next); setMessage('Workspace registered and local Agent started.'); } catch (error) { setMessage(String(error)); } }}>Start local Agent</button><button onClick={() => void invoke('open_dashboard')}>Open Dashboard</button><button onClick={async () => { try { const result = await invoke<{ approvalUrl: string; publicFingerprint: string }>('request_pairing'); setMessage(`Pairing request created for ${result.publicFingerprint}. Approve it in the Dashboard.`); await invoke('open_dashboard'); } catch (error) { setMessage(String(error)); } }}>Connect to Fleet</button></article>}

...
export function NativeAdoption({ client, request, locale, preferences }: {
  ...
  return <div className="shell native-demo">
    ...
    <aside className="context"><div className="eyebrow">{t('Native identity', '原生身份')}</div>
      ...
      <h3>{t('Native activity', '原生活动')}</h3><ul className="activity">{thread?.activity.slice(-8).map(item => <li key={item.id}>{item.status} · {item.text}</li>)}</ul>
      <details><summary>{t('Daemon and capabilities', '守护进程与能力')}</summary><pre>{JSON.stringify({ daemon: snapshot?.daemon, compatibility: snapshot?.compatibility, incarnation: snapshot?.incarnation, createdNativeThread: false }, null, 2)}</pre></details>

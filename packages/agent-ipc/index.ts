...
export type AgentIpcCommand = 'status' | 'config.get' | 'config.set' | 'runtime.list' | 'runtime.setSharing' | 'pairing.request' | 'diagnostics' | 'drain';
...
export function parseAgentIpcRequest(value: unknown): AgentIpcRequest {
  ...
  requireThat(item.v === AGENT_IPC_VERSION && typeof item.token === 'string' && /^[0-9a-f]{64}$/.test(item.token) && typeof item.command === 'string' && ['status', 'config.get', 'config.set', 'runtime.list', 'runtime.setSharing', 'pairing.request', 'diagnostics', 'drain'].includes(item.command), 'AGENT_IPC_REQUEST_INVALID');

...
export type Intent = {
  ...
  { family: 'sessionLane.acquireControl' | 'sessionLane.releaseControl' | 'sessionLane.continue'; body: Record<string, never> } |
...
export type Receipt = { edgeCommandId: string; status: 'SUCCEEDED' | 'REJECTED' | 'AMBIGUOUS_EFFECT' | 'DISPATCHED'; code: string; nativeThreadId: string | null; nativeTurnId: string | null; nativeRequestId: string | null; nativeProcessId: number | null; nativeInstanceId: string | null };
...
export type Lane = { sessionId: string; laneId: string; segmentId: string; title: string; fence: Fence; state: string; nativeThreadId: string | null; nativeTurnId: string | null; transcript: { role: 'user' | 'assistant' | 'system'; text: string }[] };
export type CommandRecord = { command: FleetCommand; plan: Plan; status: string; receipt: Receipt | null };
export type Snapshot = { status: string; target: Target; root: string; registered: boolean; lanes: Lane[]; commands: CommandRecord[]; cursor: string };
...
const families = ['workspace.register', 'logicalSession.create', 'sessionLane.acquireControl', 'sessionLane.releaseControl', 'sessionLane.continue', 'turn.submit'] as const;
...
const intent = { oneOf: families.map(family => obj({ ...intentBase, family: literal(family), body: family === 'workspace.register' ? obj({ root: str() }) : family === 'logicalSession.create' ? obj({ title: { type: 'string', minLength: 1, maxLength: 80 } }) : family === 'turn.submit' ? obj({ text: { type: 'string', minLength: 1, maxLength: 16000 } }) : obj({}) })) };
...
const receipt = obj({ edgeCommandId: uuid, status: { enum: ['SUCCEEDED', 'REJECTED', 'AMBIGUOUS_EFFECT', 'DISPATCHED'] }, code: str(120), nativeThreadId: nullable(str(200)), nativeTurnId: nullable(str(200)), nativeRequestId: nullable(uuid), nativeProcessId: nullable({ type: 'integer', minimum: 1 }), nativeInstanceId: nullable(uuid) });

...
test('an approved enrolled carriage replaces a detached bootstrap carriage', async () => {
  ...
  const bootstrapToken = randomBytes(32).toString('hex'); const hcpToken = randomBytes(32).toString('hex');
  const remote = new RemoteAdoptionPortProxy(target, () => { throw new Error('HCP_NOT_CONNECTED'); });
  const hub = await startHub({ port, target, root: 'V:\\device-fixture', sid: 'fixture', principal: 'fixture', sessionId: 1, stateDirectory: mkdtempSync(path.join(tmpdir(), 'fleet-device-replace-')), webDirectory: path.resolve('dist/web'), hcpToken, bootstrapToken }, remote); const origin = `http://127.0.0.1:${port}`;
  ...
  try {
    ...
    const enrolled = await new Promise<WebSocket>((resolve, reject) => { const ws = new WebSocket(`ws://127.0.0.1:${port}/hcp/v1/connect`, 'fleetsplice.hcp.v1', { origin }); ws.once('error', reject); ws.on('open', () => ws.send(JSON.stringify({ v: 1, kind: 'hello', connectionId: target.connectionId, target, identity, recovered: false, enrollment }))); ws.on('message', bytes => { const message = JSON.parse(String(bytes)); if (message.kind === 'enrollment.challenge') ws.send(JSON.stringify({ v: 1, kind: 'enrollment.proof', connectionId: target.connectionId, target, hostId: material.hostId, proof: signEnrollmentChallenge(material, message.challenge) })); if (message.kind === 'ready') resolve(ws); if (message.kind === 'adoption.request') ws.send(JSON.stringify({ v: 1, kind: 'adoption.response', connectionId: target.connectionId, target, requestId: message.requestId, ok: true, code: null, body: { state: 'READY', threads: [] } })); }); });
    assert.equal((await remote.snapshot({ discover: true })).state, 'READY');
    enrolled.close();
  } finally { await hub.close(); }
});

test('Gateway shutdown terminally disposes its process-scoped remote AdoptionPort', async () => {
  const probe = createServer(); await new Promise<void>(resolve => probe.listen(0, '127.0.0.1', resolve)); const port = (probe.address() as { port: number }).port; await new Promise<void>(resolve => probe.close(() => resolve()));
  const target: Target = { authorityId: randomUUID(), hubRuntimeId: randomUUID(), edgeRuntimeId: randomUUID(), connectionId: randomUUID(), hubRecoveryGeneration: '1', edgeRecoveryGeneration: '1', hostId: randomUUID(), hostGeneration: '1', environmentId: randomUUID(), environmentGeneration: '1', workspaceId: randomUUID(), workspaceGeneration: '1', rootIdentity: 'c'.repeat(64), agentBindingId: randomUUID(), executionBindingId: randomUUID(), providerBindingId: randomUUID() };
  const remote = new RemoteAdoptionPortProxy(target, () => { throw new Error('HCP_NOT_CONNECTED'); });
  const hub = await startHub({ port, target, root: 'V:\\device-fixture', sid: 'fixture', principal: 'fixture', sessionId: 1, stateDirectory: mkdtempSync(path.join(tmpdir(), 'fleet-device-dispose-')), webDirectory: path.resolve('dist/web'), hcpToken: randomBytes(32).toString('hex'), bootstrapToken: randomBytes(32).toString('hex') }, remote);
  await hub.close();
  await assert.rejects(remote.snapshot({}), /GATEWAY_DISPOSED/);
});
